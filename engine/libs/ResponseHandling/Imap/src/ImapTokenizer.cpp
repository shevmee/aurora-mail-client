...
namespace aurora::mail::imap::parser
{
  ...
  std::expected<Value, std::string> Tokenizer::nextValue() {
    ...
    if (c == '{')
    {
      ...
      if (!literal_result) return std::unexpected(literal_result.error());
  ...
  std::expected<std::string, std::string> Tokenizer::readQuoted()
  {
    ...
    while (!this->isEof()) {
      char c = this->get();
      if (c == '\\') {
        if (this->isEof()) return std::unexpected(std::format("Unterminated escape sequence at position {}", pos_));
