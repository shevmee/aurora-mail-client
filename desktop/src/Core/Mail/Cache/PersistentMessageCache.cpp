...
namespace aurora::mail::app::cache
{
  ...
  namespace
  {
    ...
    void restrictPermissionsWin(const QString& path)
    {
      ...
      {
        qWarning() << "PersistentMessageCache: OpenProcessToken failed:" << static_cast<unsigned long>(GetLastError());
        return;
      }
      ...
      {
        qWarning() << "PersistentMessageCache: GetTokenInformation failed:" << static_cast<unsigned long>(GetLastError());
        CloseHandle(token);
        return;
      }
      CloseHandle(token);
      ...
      {
        qWarning() << "PersistentMessageCache: SetEntriesInAcl failed:" << static_cast<unsigned long>(createResult);
        return;
      }
      ...
      {
        qWarning() << "PersistentMessageCache: SetNamedSecurityInfo failed:" << static_cast<unsigned long>(setResult);
      }
      LocalFree(acl);
    }
#endif
  ...
  QString PersistentMessageCache::filePathFor(const QString& accountId)
  {
    ...
    return dir.filePath(
        QString::fromLatin1(kCacheSubdir) + QLatin1Char('/') + QString::fromLatin1(digest.toHex().left(32)) +
        QStringLiteral(".db"));
  }
  ...
  PersistentMessageCache::~PersistentMessageCache()
  {
    ...
    const QString name =
        baseConnectionName_ + QStringLiteral("-") + QString::number(reinterpret_cast<quintptr>(QThread::currentThreadId()));
  ...
  void PersistentMessageCache::put(const CachedMessage& entry)
  {
    ...
    const qint64 cachedAtMs =
        entry.cachedAt.isValid() ? entry.cachedAt.toMSecsSinceEpoch() : QDateTime::currentMSecsSinceEpoch();
  ...
  void PersistentMessageCache::invalidateMailbox(const QString& mailbox)
  {
    ...
    QSqlQuery q(db);
    q.prepare(QStringLiteral("DELETE FROM messages WHERE account_id=:a AND mailbox=:m"));
  ...
  void PersistentMessageCache::enforceBudget_locked()
  {
    ...
    while (over > 0)
    {
      ...
      {
        doomed.emplace_back(
            sel.value(0).toString(), sel.value(1).toString(), sel.value(2).toULongLong(), sel.value(3).toULongLong());
