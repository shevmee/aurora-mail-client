...
void ImapCommandQueue::enqueue(ImapOperation op)
{
  ...
  switch (op.type)
  {
    ...
    case ImapOpType::LoadEmail:
      std::erase_if(queue_, [](const ImapOperation& o) { return o.type == ImapOpType::LoadEmail; });
      queue_.push_back(std::move(op));
      break;

    case ImapOpType::FetchMailboxPage:
      std::erase_if(queue_, [](const ImapOperation& o) { return o.type == ImapOpType::FetchMailboxPage; });
      queue_.push_back(std::move(op));
      break;

    default: queue_.push_back(std::move(op)); break;
  }
}
