...
namespace aurora::mail::app::email
{
  ...
  namespace
  {
    ...
    QStringList tokenizeEnvelopeFields(const QString& env)
    {
      ...
      while (i < env.length())
      {
        ...
        else if (env.mid(i, 3) == QStringLiteral("NIL") && (i + 3 == env.length() || !env[i + 3].isLetterOrNumber()))
        {
  ...
  EmailSummary EmailSummary::fromReceivedMessage(const ReceivedMailMessage& msg, const QString& uid)
  {
    ...
    // Format sender
    summary.from = QString::fromStdString(msg.from.getName().empty() ? msg.from.getAddress() : msg.from.getName());
      ...
      if (envStart != -1)
      {
          ...
          if (dateEnd != -1)
          {
            ...
            if (subjectStart != -1)
            {
              ...
              if (subjectEnd != -1)
              {
                ...
                {
                  summary.subject = QString::fromStdString(mime_reader::decodeHeaderValue(rawSubject.toStdString()));
                }
                else
                {
        ...
        if (fromMatch.hasMatch())
        {
          ...
          if (!name.isEmpty())
          {
            ...
            if (name.contains("=?"))
            {
              summary.from = QString::fromStdString(mime_reader::decodeHeaderValue(name.toStdString()));
            }
            else
            {
              summary.from = name;
  ...
  std::string EmailParser::extractBodyFromFetch(const std::string& response)
  {
    ...
    if (contentStart < response.size() && response[contentStart] == '\r')
      contentStart++;
  ...
  std::optional<ReceivedMailMessage> EmailParser::parseRawMessage(const std::string& rawMessage)
  {
    ...
    if (result.has_value())
    {
      qDebug() << "[EmailParser] MIME parse successful - subject:" << QString::fromStdString(result->subject)
               << "text_body size:" << result->text_body.size() << "html_body size:" << result->html_body.size()
               << "attachments:" << result->attachments.size();
      return result.value();
    }

    qWarning() << "[EmailParser] MIME parse failed:" << QString::fromStdString(result.error().toString());
    return std::nullopt;
  }
    ...
    if (parsed)
    {
      ...
      if (!parsed->from.getName().empty())
      {
        from = QString("%1 <%2>").arg(
            QString::fromStdString(parsed->from.getName()), QString::fromStdString(parsed->from.getAddress()));
      }
      else
      {
      ...
      {
        const QString plain = TextSanitizer::sanitizePlainText(QString::fromStdString(parsed->text_body));
  ...
  ParsedEmailContent EmailParser::parseFullEmailContent(const std::string& response)
  {
    ...
    if (parsed)
    {
      ...
      for (const auto& att : parsed->attachments)
      {
        ...
        info.data = QByteArray(reinterpret_cast<const char*>(att.data.data()), static_cast<int>(att.data.size()));
        info.isInline = att.is_inline;
        result.attachments.append(info);

        qDebug() << "[EmailParser] Found attachment:" << info.filename << "size:" << info.size << "bytes"
