...
namespace aurora::mail::app::email
{
  ...
  struct ParsedEmailContent
  {
    ...
    bool isValid() const
    {
      return !subject.isEmpty() || !from.isEmpty() || !body.isEmpty();
    }
  };
  ...
  struct EmailSummary
  {
     ...
     */
    static EmailSummary fromReceivedMessage(const aurora::mail::common::mail::ReceivedMailMessage& msg, const QString& uid);
  };
  ...
  class EmailParser
  {
   public:
     ...
     */
    static std::optional<aurora::mail::common::mail::ReceivedMailMessage> parseRawMessage(const std::string& rawMessage);
