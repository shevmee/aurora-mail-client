...
void TextSanitizer::fixInvalidCssColors(QString& html)
{
    ...
    static const QRegularExpression fourCharColor(
    ...
    while (it.hasNext()) {
        ...
        replacements.append({static_cast<int>(match.capturedStart()), static_cast<int>(match.capturedLength()), replacement});
    }
    ...
    static const QRegularExpression eightCharTransparent(
        R"(#[0-9A-Fa-f]{6}00(?![0-9A-Fa-f]))"
    );
