...
namespace aurora::mail::app::config
{
  ...
  namespace
  {
    ...
    std::expected<aurora::mail::common::config::LoggerConfig, std::string> parseLoggerConfig(const QJsonObject& obj)
    {
      ...
      cfg.flushIntervalMsgs = flush.isDouble() ? static_cast<std::size_t>(flush.toDouble()) : static_cast<std::size_t>(16);
      return cfg;
    }
  ...
  std::expected<AppConfig, std::string> loadAppConfig(const QString& filename)
  {
    ...
    {
      return std::unexpected(std::string("Failed to parse JSON config: ") + parseError.errorString().toStdString());
    }
    if (!doc.isObject())
    {
