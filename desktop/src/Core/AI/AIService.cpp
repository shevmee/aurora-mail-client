...
namespace aurora::mail::app::ai
{
  ...
  AIService::AIService(QObject* parent) : QObject(parent), m_networkManager(new QNetworkAccessManager(this))
  {
    QSettings settings;
...
Respond with ONLY the corrected text, no explanations.)");
    ...
    sendRequest(systemPrompt, text, [this](const Result& result) { emit grammarCheckCompleted(result); });
  }
...
Respond with ONLY the concise version, no explanations.)");
      ...
      const std::function<void(const Result&)>& callback)
  {
    if (!isConfigured())
    {
      ...
      result.errorMessage = QStringLiteral("AI API key not configured. Open Settings and add your Google AI Studio key.");
