...
class PasswordCredentialsStorage
{
 public:
  ...
  struct Credentials
  {
    ...
    {
      return !email.isEmpty() && !password.isEmpty() && !imapServer.isEmpty() && !smtpServer.isEmpty();
    }
  };
  ...
  {
    ServerHints h;
    h.imapServer = settings_.value(QStringLiteral("imapServer"), QStringLiteral("imap.gmail.com")).toString();
    h.imapPort = static_cast<quint16>(settings_.value(QStringLiteral("imapPort"), 993).toUInt());
    h.smtpServer = settings_.value(QStringLiteral("smtpServer"), QStringLiteral("smtp.gmail.com")).toString();
    h.smtpPort = static_cast<quint16>(settings_.value(QStringLiteral("smtpPort"), 587).toUInt());
    return h;
  }
