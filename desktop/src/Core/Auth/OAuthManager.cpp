...
void OAuthManager::setupConnections() {
    ...
    connect(&redirect_server_, &OAuthRedirectServer::authCodeReceived,
        this, &OAuthManager::handleAuthorizationCode);
    connect(&redirect_server_, &OAuthRedirectServer::errorReceived,
            this, &OAuthManager::authenticationFailed);
    connect(&redirect_server_, &OAuthRedirectServer::serverError,
            this, &OAuthManager::authenticationFailed);
}
...
void OAuthManager::ensureValidAccessToken(std::function<void(std::optional<std::string>)> callback)
{
    ...
    refreshAccessTokenAsync([this, cb = std::move(callback)](bool success) mutable {
        if (!success) {
            cb(std::nullopt);
            return;
        }
...
{
    return [this]() -> std::optional<std::string> {
...
{
    return !tokens_.accessToken.isEmpty() &&
           (!tokens_.isExpired() || tokens_.canRefresh());
}
