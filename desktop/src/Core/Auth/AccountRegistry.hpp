...
public:
    ...
    [[nodiscard]] std::optional<Account> findByEmail(const QString& email) const
    {
        ...
        const auto it = std::find_if(
            all.begin(), all.end(),
            [&email](const Account& a) { return a.email == email; });
        if (it == all.end()) {
            return std::nullopt;
        }
        return *it;
    ...
    void upsert(const Account& account)
    {
        ...
        auto it = std::find_if(
            all.begin(), all.end(),
            [&account](const Account& a) { return a.email == account.email; });
        if (it == all.end()) {
...
private:
    ...
    [[nodiscard]] std::vector<Account> loadAll() const
    {
        ...
        const QByteArray raw =
            settings_.value(QStringLiteral("accounts")).toString().toUtf8();
        if (raw.isEmpty()) {
            return out;
        }
        QJsonParseError err{};
        ...
        for (const auto v : arr) {
            ...
            a.imapServer = o.value(QLatin1String("imapServer"))
                               .toString(QStringLiteral("imap.gmail.com"));
            a.imapPort = static_cast<quint16>(
                o.value(QLatin1String("imapPort")).toInt(993));
            a.smtpServer = o.value(QLatin1String("smtpServer"))
                               .toString(QStringLiteral("smtp.gmail.com"));
            a.smtpPort = static_cast<quint16>(
                o.value(QLatin1String("smtpPort")).toInt(587));
