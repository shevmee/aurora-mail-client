...
OAuthRedirectServer::OAuthRedirectServer(QObject* parent) : QObject(parent), server_(new QTcpServer(this))
{
  connect(server_, &QTcpServer::newConnection, this, &OAuthRedirectServer::handleNewConnection);
}
...
void OAuthRedirectServer::processRequest(QTcpSocket* socket, const QByteArray& requestData)
{
  ...
  if (!error.isEmpty())
  {
    ...
    responseBody = AuthPageRenderer::renderErrorPage(errorDesc.isEmpty() ? error : errorDesc);
  }
