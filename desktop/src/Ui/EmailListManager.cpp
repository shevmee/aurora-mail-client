...
void EmailListManager::applyFetchResults(const QVector<EmailSummary>& emails,
                                         bool append,
...
void EmailListManager::setSearchFilter(const QString& text)
{
    ...
    for (auto it = m_emailItems.begin(); it != m_emailItems.end(); ++it) {
        ...
        bool matches = text.isEmpty() ||
                       summary.from.toLower().contains(searchLower) ||
                       summary.subject.toLower().contains(searchLower) ||
                       summary.preview.toLower().contains(searchLower);
...
void EmailListManager::removeEmail(const QString& uid)
{
    ...
    if (!m_currentMailbox.isEmpty() && m_mailboxCache.contains(m_currentMailbox)) {
        ...
        cache.emails.erase(
            std::remove_if(cache.emails.begin(), cache.emails.end(),
                [&uid](const EmailSummary& e) { return e.uid == uid; }),
            cache.emails.end());
...
void EmailListManager::updateLoadMoreButton()
{
    ...
    if (m_loadMoreButton == nullptr) {
        ...
        connect(m_loadMoreButton, &QPushButton::clicked,
                this, &EmailListManager::loadMoreRequested);
    }
...
void EmailListManager::updateCache(const QString& mailbox,
