...
SessionCoordinator::SessionCoordinator(Bindings bindings, QObject* parent)
    : QObject(parent)
    , m_b(bindings)
{
}
...
void SessionCoordinator::registerActiveAccount(EAuthMethod method)
{
    ...
    a.method = (method == EAuthMethod::Password)
                   ? AccountRegistry::AuthMethod::Password
                   : AccountRegistry::AuthMethod::OAuth;
...
bool SessionCoordinator::reconnectStoredAccount(const AccountRegistry::Account& account)
{
    ...
    if (account.method == AccountRegistry::AuthMethod::OAuth) {
        ...
        m_b.oauthManager.setCredentials(
...
void SessionCoordinator::populateAccountMenu(QMenu* menu)
{
    ...
    for (const auto& acc : accounts) {
        ...
        connect(act, &QAction::triggered, this,
                [this, email]() { emit switchAccountRequested(email); });
    }
    ...
    connect(addAct, &QAction::triggered, this,
            [this]() { emit addAccountRequested(); });

    menu->addSeparator();
    QAction* signOutAct = menu->addAction(tr("Sign out of this account"));
    connect(signOutAct, &QAction::triggered, this,
