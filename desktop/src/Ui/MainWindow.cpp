    ...
    , m_mailSessionSignals(std::make_unique<MailSessionSignals>(this))
{
    ...
    m_emailListManager = new EmailListManager(ui->mailItemsLayout,
                                              ui->MailListContainer,
                                              ui->EmailActionsBar,
                                              this);
    loadStylesheet();
    applyMacSafeCursors();
    ...
    , m_mailSessionSignals(std::make_unique<MailSessionSignals>(this))
{
    ...
    m_mailSession = std::make_unique<MailSessionService>(
        ...
        [this](const ImapOperation& op) -> boost::asio::awaitable<void> {
            return dispatchImapOperation(op);
        });
...
void MainWindow::applyMacSafeCursors()
{
    ...
    const QList<QWidget*> widgets =
        findChildren<QWidget*>(QString(), Qt::FindChildrenRecursively);
...
void MainWindow::setupConnections()
{
    ...
    connect(m_session.get(), &SessionCoordinator::statusMessageRequested,
            ui->StatusLabel, &QLabel::setText);
    connect(m_session.get(), &SessionCoordinator::uiEnableRequested,
            this, &MainWindow::setUIEnabled);
    connect(m_session.get(), &SessionCoordinator::errorMessageRequested,
            this, &MainWindow::showError);

    // Hand-offs back to the host's async network paths and view transitions.
    connect(m_session.get(), &SessionCoordinator::passwordSignInRequested,
            this, &MainWindow::authenticateWithPasswordAsync);
    // Queued so the OAuth completion runs once the current call stack
    // unwinds (matches the original QTimer::singleShot(0, ...) pattern).
    connect(m_session.get(), &SessionCoordinator::oauthCompletionRequested,
            this, &MainWindow::onOAuthAuthenticated, Qt::QueuedConnection);
    connect(m_session.get(), &SessionCoordinator::sessionTeardownRequested,
            this, &MainWindow::teardownActiveSession);
    connect(m_session.get(), &SessionCoordinator::loginPageRequested,
            this, [this]() {
    ...
    connect(m_session.get(), &SessionCoordinator::switchAccountRequested,
            this, &MainWindow::onSwitchAccountSelected);
    connect(m_session.get(), &SessionCoordinator::addAccountRequested,
            this, &MainWindow::onAddAccountSelected);
    connect(m_session.get(), &SessionCoordinator::signOutRequested,
            this, &MainWindow::onSignOutButtonClicked);

    // Login
    connect(ui->LogInButton, &QPushButton::clicked, this, &MainWindow::onLogInButtonClicked);
    connect(ui->UsePasswordButton, &QPushButton::clicked,
            this, &MainWindow::onUsePasswordLoginClicked);
    connect(ui->BackToOAuthButton, &QPushButton::clicked,
            this, &MainWindow::onBackToOAuthClicked);
    connect(ui->PasswordSignInButton, &QPushButton::clicked,
            this, &MainWindow::onPasswordSignInClicked);
    // Pressing Enter in either text field submits the password login form.
    connect(ui->PasswordLine, &QLineEdit::returnPressed,
            this, &MainWindow::onPasswordSignInClicked);
    connect(ui->LoginLine, &QLineEdit::returnPressed,
            this, &MainWindow::onPasswordSignInClicked);
    ...
    // OAuth signals
    connect(m_oauthManager.get(), &OAuthManager::authenticated, 
            this, &MainWindow::onOAuthAuthenticated);
    connect(m_oauthManager.get(), &OAuthManager::authenticationFailed, 
            this, &MainWindow::onOAuthFailed);
    ...
    if (m_emailListManager != nullptr) {
        connect(m_emailListManager, &EmailListManager::emailSelected,
                this, &MainWindow::onEmailItemClicked);
        connect(m_emailListManager, &EmailListManager::loadMoreRequested,
                this, &MainWindow::onLoadMoreClicked);
    }

    connect(ui->MailListArea->verticalScrollBar(), &QAbstractSlider::valueChanged, this,
    ...
    connect(m_accountMenu, &QMenu::aboutToShow,
            this, &MainWindow::rebuildAccountMenu);
...
void MainWindow::initializeLoginPage()
{
    ...
    if (!oauthConfigured) {
        ...
        ui->LoginHintLabel->setText(
            QStringLiteral("OAuth is not configured. Use email and password to sign in."));
    ...
    if (!PasswordCredentialsStorage::isSecure()) {
        ...
        const QString warn = QStringLiteral(
            "Secure storage unavailable: passwords will not be remembered after restart.");
        ui->LoginHintLabel->setText(current.isEmpty() ? warn
...
void MainWindow::showLoginMethodPage(ELoginMethodPage page)
{
    ...
    if (page == ELoginMethodPage::PasswordPage) {
        ...
        if (auto stored = m_passwordCredentials.load(ui->LoginLine->text());
            stored.has_value()) {
            ui->PasswordLine->setText(stored->password);
            ui->ImapServerLine->setText(
                QStringLiteral("%1:%2").arg(stored->imapServer).arg(stored->imapPort));
            ui->SmtpServerLine->setText(
                QStringLiteral("%1:%2").arg(stored->smtpServer).arg(stored->smtpPort));
        }
...
void MainWindow::setupInboxSplitter()
{
    ...
    connect(m_inboxSplitter, &QSplitter::splitterMoved, this, [this](int /*pos*/, int /*index*/) {
        QSettings s;
        QList<QVariant> list;
...
void MainWindow::setupEmailReaderSplitter()
{
    ...
    ui->EmailBodyText->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    {
        ...
        f.setStyleStrategy(QFont::StyleStrategy(
            int(QFont::PreferDefault) | int(QFont::NoFontMerging)));
    ...
    connect(m_emailReaderSplitter, &QSplitter::splitterMoved, this, [this](int, int) {
        QSettings s;
        QList<QVariant> list;
...
void MainWindow::onOAuthAuthenticated()
{
    ...
    m_oauthManager->ensureValidAccessToken([this](std::optional<std::string> tokenOpt) {
        if (!tokenOpt) {
            setUIEnabled(true);
...
void MainWindow::authenticateWithOAuthAsync(const std::string& accessToken)
{
    ...
    boost::asio::co_spawn(
        *m_ioContext,
        [this, strand, imapClient, smtpClient, username, imapServer, imapPort, smtpServer, smtpPort,
         accessToken]() mutable -> boost::asio::awaitable<void> {
            auto finishUi = [this](bool ok) {
                QMetaObject::invokeMethod(
                    this,
                    [this, ok]() { completeMailAuthenticationUi(ok); },
                    Qt::QueuedConnection);
            };
            ...
            try {
                    ...
                    *strand,
                    [imapClient, imapServer, imapPort, username, accessToken]() -> boost::asio::awaitable<bool> {
                        ...
                        if (!imapConnectResult.has_value()) {
                            qWarning() << "IMAP connect failed:"
                                       << QString::fromStdString(imapConnectResult.error().toString());
                            co_return false;
                        }
                        qDebug() << "IMAP connected successfully";

                        aurora::mail::common::TagGenerator tagGen;
                        aurora::mail::imap::command::AuthXOAuth2 imapAuth{
                            tagGen.next(),
                            username,
                            accessToken
                        };
                        ...
                        if (!imapAuthResult.has_value()) {
                            qWarning() << "IMAP auth failed:"
                                       << QString::fromStdString(imapAuthResult.error().toString());
                            co_return false;
                        }
                ...
                if (!smtpConnectResult.has_value()) {
                    qWarning() << "SMTP connect failed:"
                               << QString::fromStdString(smtpConnectResult.error().toString());
                    publishState(ImapSessionLinkState::Disconnected);
                    finishUi(false);
                    co_return;
                }
                ...
                aurora::mail::smtp::command::AuthXOAuth2 smtpAuth{
                    username,
                    accessToken
                };
                ...
                if (!smtpAuthResult.has_value()) {
                    qWarning() << "SMTP auth failed:"
                               << QString::fromStdString(smtpAuthResult.error().toString());
                    publishState(ImapSessionLinkState::Disconnected);
                    finishUi(false);
                    co_return;
                }
                ...
                QMetaObject::invokeMethod(
                    this,
                    [this]() { m_currentAuthMethod = EAuthMethod::OAuth; },
                    Qt::QueuedConnection);
                finishUi(true);
 ...
 */
std::optional<std::pair<QString, quint16>> parseHostPort(const QString& raw,
                                                        quint16 defaultPort)
{
...
void MainWindow::onPasswordSignInClicked()
{
    ...
    if (creds.email.isEmpty() || !isValidEmail(creds.email)) {
        showError(QStringLiteral("Invalid email"),
                  QStringLiteral("Please enter a valid email address."));
        ui->LoginLine->setFocus();
        return;
    }
    if (creds.password.isEmpty()) {
        showError(QStringLiteral("Password required"),
                  QStringLiteral("Please enter your password or app password."));
    ...
    if (!imapParsed) {
        showError(QStringLiteral("Invalid IMAP server"),
    ...
    if (!smtpParsed) {
        showError(QStringLiteral("Invalid SMTP server"),
...
void MainWindow::authenticateWithPasswordAsync(PasswordCredentialsStorage::Credentials creds)
{
    ...
    boost::asio::co_spawn(
        *m_ioContext,
        [this, strand, imapClient, smtpClient, username, password,
         ...
         credsToPersist]() mutable -> boost::asio::awaitable<void> {
            ...
            try {
                ...
                const bool imapOk = co_await boost::asio::co_spawn(
                    *strand,
                    [imapClient, imapServer, imapPort, username,
                     password]() -> boost::asio::awaitable<bool> {
                        co_await imapClient->closeConnection();
                        imapClient->reset();
                        ...
                        if (!connectResult.has_value()) {
                            qWarning() << "IMAP connect failed:"
                                       << QString::fromStdString(connectResult.error().toString());
                            co_return false;
                        }
                        ...
                        aurora::mail::imap::command::AuthPlain auth{
                            tagGen.next(), username, password};
                        auto authResult = co_await imapClient->asyncAuthenticate(auth);
                        if (!authResult.has_value()) {
                            qWarning() << "IMAP password auth failed:"
                                       << QString::fromStdString(authResult.error().toString());
                            co_return false;
                        }
                ...
                auto smtpAuthResult = co_await smtpClient->asyncAuthenticate(smtpAuth);
                if (!smtpAuthResult.has_value()) {
                    qWarning() << "SMTP password auth failed:"
                               << QString::fromStdString(smtpAuthResult.error().toString());
                    publishState(ImapSessionLinkState::Disconnected);
                    finishUi(false);
                    co_return;
...
void MainWindow::ensureMessageCacheForCurrentAccount()
{
    ...
    if (!key.has_value()) {
        qInfo() << "Message cache: persistent tier disabled for" << m_currentUser
                << "(no secure key storage available)";
        return;
    }
    auto persistent = std::make_unique<aurora::mail::app::cache::PersistentMessageCache>(
        m_currentUser, *key);
    aurora::mail::app::cache::AesGcmCipher::secureWipe(*key);

    if (!persistent->isEnabled()) {
        qWarning() << "Message cache: failed to open persistent tier for" << m_currentUser
                   << "; staying memory-only";
        return;
    }
...
void MainWindow::onSwitchAccountSelected(const QString& email)
{
    ...
    if (!m_session->reconnectStoredAccount(*target)) {
        ...
        ui->StatusLabel->setText(
            tr("Could not resume %1 — please sign in again.").arg(email));
    }
}
    ...
    boost::asio::co_spawn(
        *m_ioContext,
        [this, strand, imapClient, after = std::move(afterAppliedOnQtThread)]() mutable -> boost::asio::awaitable<void> {
            std::vector<aurora::mail::imap::MailboxInfo> folders =
                co_await boost::asio::co_spawn(
                    *strand,
                    [imapClient]() -> boost::asio::awaitable<std::vector<aurora::mail::imap::MailboxInfo>> {
                        ...
                        qWarning() << "Failed to list folders:"
...
void MainWindow::applyFolderList(std::vector<aurora::mail::imap::MailboxInfo> folders)
{
    ...
    for (const auto& folder : folders) {
        ...
        if (folder.hasAttribute("\\Sent") || folder.hasAttribute("\\Trash") ||
            folder.hasAttribute("\\Drafts")) {
            continue;
        }
...
{
    if (!m_imapClient.lock() || (m_ioContext == nullptr)) return;
...
{
    if ((m_emailListManager == nullptr) || m_emailListManager->selectedUid().isEmpty()) return;
...
void MainWindow::onDeleteButtonClicked()
{
    ...
    if (QMessageBox::question(this, "Delete Email", "Are you sure you want to delete this email?",
...
void MainWindow::onMoveButtonClicked()
{
    ...
    bool ok;
    QString destination = QInputDialog::getItem(
        this, "Move to Folder", "Select destination folder:",
        folders, 0, false, &ok);
...
void MainWindow::onSendButtonClicked()
{
    ...
    builder.from(MailAddress{m_currentUser.toStdString()})
    ...
    m_oauthManager->ensureValidAccessToken(
        [this, smtpClient, mailMessage = std::move(mailMessage)](std::optional<std::string> tokenOpt) mutable {
            if (!tokenOpt) {
                QMetaObject::invokeMethod(
                    ...
                    [this]() {
                        ...
                        showError(
                            "Send Failed",
                            "Not signed in or the session expired. Please sign in again.");
                    },
                    Qt::QueuedConnection);
                return;
            }
            ...
            boost::asio::co_spawn(
                *m_ioContext,
                [this, smtpClient, mailMessage = std::move(mailMessage), accessToken, username, smtpServer,
                 smtpPort]() mutable -> boost::asio::awaitable<void> {
                    ...
                    if (!ok) {
                        qWarning() << "Failed to send email:"
                                   << QString::fromStdString(sendResult.error().toString());
                    } else {
...
void MainWindow::onAIAssistButtonClicked()
{
    ...
    if (!m_aiService->isConfigured()) {
        showError(
            "AI Assist",
            "Add your Google AI Studio API key in Settings (sidebar) to use AI features.");
    ...
    connect(m_aiService.get(), &AIService::grammarCheckCompleted, dialog,
    ...
    connect(m_aiService.get(), &AIService::textImprovementCompleted, dialog,
            [dialog](const AIService::Result& result) {
        if (result.success) {
            dialog->setImprovedText(result.improvedText);
            dialog->showComparison();
    ...
    connect(dialog, &aurora::mail::ui::AIAssistantDialog::modeChanged, this,
            [this, body](aurora::mail::ui::AIAssistantDialog::Mode mode) {
        switch (mode) {
            case aurora::mail::ui::AIAssistantDialog::Mode::GrammarCheck:
                m_aiService->checkGrammar(body);
                break;
            case aurora::mail::ui::AIAssistantDialog::Mode::ImproveWriting:
                m_aiService->improveWriting(body);
                break;
            case aurora::mail::ui::AIAssistantDialog::Mode::MakeFormal:
                m_aiService->makeMoreFormal(body);
                break;
            case aurora::mail::ui::AIAssistantDialog::Mode::MakeConcise:
                m_aiService->makeConcise(body);
                break;
        }
    });
...
void MainWindow::selectAttachments()
{
    ...
    ui->AttachmentLabel->setText(m_selectedAttachments.isEmpty() 
        ? "" 
        : QString("%1 file(s) attached").arg(m_selectedAttachments.size()));
}
...
void MainWindow::displayAttachments()
{
    ...
    for (int i = 0; i < m_currentEmailAttachments.size(); ++i) {
        ...
        QPushButton* btn = new QPushButton(
            QString("%1 (%2)").arg(att.filename, sizeStr), this);
...
ImapSessionCallbacks MainWindow::buildImapSessionCallbacks()
{
    ...
    c.onIdleRawNotification = [this](const QString& notification) {
        QMetaObject::invokeMethod(
            this,
            [this, notification]() { handleIdleServerNotification(notification); },
            Qt::QueuedConnection);
    };
    ...
    c.onPumpIdleWaitTimeout = [this]() {
        QMetaObject::invokeMethod(
            this,
            [this]() { showStatus("Mail connection busy; try again.", 5000); },
            Qt::QueuedConnection);
    };
    c.onIdleLoopFinished = [this]() {
        QMetaObject::invokeMethod(
            this,
            [this]() { emit idleLoopStopped(); },
            Qt::QueuedConnection);
    };
    return c;
}
...
void MainWindow::handleIdleServerNotification(const QString& notification)
{
    ...
    if (notification.contains(QStringLiteral("EXISTS"), Qt::CaseInsensitive)) {
        QRegularExpression existsRegex(
            R"(\*\s+(\d+)\s+EXISTS)",
...
boost::asio::awaitable<void> MainWindow::dispatchImapOperation(const ImapOperation& op)
{
    ...
    switch (op.type) {
        case ImapOpType::SelectMailbox: {
            ...
            if (result.has_value()) {
                qDebug() << "Mailbox selected:" << QString::fromStdString(mailboxStr) << "with"
                         << result.value().exists << "messages";
                messageCount = static_cast<int>(result.value().exists);
                newUidValidity = static_cast<quint32>(result.value().uidvalidity);
            } else {
                qWarning() << "Failed to select mailbox:"
                           << QString::fromStdString(result.error().toString());
            }
            ...
            QMetaObject::invokeMethod(
                ...
                [this, messageCount, newUidValidity, mailbox]() {
                    if (messageCount < 0) {
                        ...
                        if (newUidValidity != 0 && m_currentMailboxUidValidity != 0
                            && newUidValidity != m_currentMailboxUidValidity)
                        {
                            qWarning() << "UIDVALIDITY changed for" << mailbox
                                       << "from" << m_currentMailboxUidValidity
                                       << "to" << newUidValidity << "; invalidating cache";
        ...
        case ImapOpType::FetchMailboxPage:
            co_await fetchMailboxPageAwaitable(op.boolParam, -1);
            break;
        ...
        case ImapOpType::LoadEmail: {
            ...
            if (fetchResult.has_value()) {
                ...
                qWarning() << "Failed to fetch email content:"
                           << QString::fromStdString(fetchResult.error().toString());
            }
            ...
            QMetaObject::invokeMethod(
                this,
                [this, parsed, uid]() { applyParsedEmailBodyOnQt(parsed, uid); },
                Qt::BlockingQueuedConnection);
            break;
        }
        case ImapOpType::MarkRead: {
            std::string uidStr = op.param1.toStdString();
            std::string flagAction =
                op.boolParam ? "+FLAGS.SILENT (\\Seen)" : "-FLAGS.SILENT (\\Seen)";
            auto result = co_await imapClient->asyncUidStoreMail(uidStr, flagAction);
            if (!result.has_value()) {
                qWarning() << "Failed to update read status:"
                           << QString::fromStdString(result.error().toString());
            }
            break;
        }
        case ImapOpType::Delete: {
            ...
            if (!storeResult.has_value()) {
                ...
                if (!expungeResult.has_value()) {
                    qWarning() << "Failed to expunge email:"
        ...
        case ImapOpType::Move: {
            ...
            QMetaObject::invokeMethod(
                ...
                [this, destinationMailbox]() {
                    showStatus(QString("Moving to %1...").arg(destinationMailbox), 0);
                },
                Qt::BlockingQueuedConnection);
            ...
            if (moveResult.has_value()) {
                ...
                if (copyResult.has_value()) {
                    ...
                    QMetaObject::invokeMethod(
                        this,
                        [this]() { showError("Move Failed", "Failed to move the email."); },
                        Qt::BlockingQueuedConnection);
                }
            }
            break;
        ...
        case ImapOpType::ToggleFlag: {
            ...
            auto result = co_await imapClient->asyncUidStoreMail(uidStr, flagAction);
            if (!result.has_value()) {
                qWarning() << "Failed to toggle flag:"
                           << QString::fromStdString(result.error().toString());
            }
            break;
        }
...
boost::asio::awaitable<void> MainWindow::fetchMailboxPageAwaitable(bool append, int serverMessageCountOrNeg1)
{
    ...
    if (!append) {
        QMetaObject::invokeMethod(
            this,
            [this]() { m_mailListScrollReadyForMore = true; },
            Qt::BlockingQueuedConnection);
    }
    ...
    QMetaObject::invokeMethod(
        this,
        [this, emails, append, startSeq]() {
            applyParsedEmailListOnQt(emails, append, startSeq);
        },
        Qt::BlockingQueuedConnection);
...
QPushButton* MainWindow::createFolderButton(const QString& folderName, const QString& displayName)
{
    ...
    connect(button, &QPushButton::clicked, this, [this, folderName]() {
        onFolderButtonClicked(folderName);
    });
