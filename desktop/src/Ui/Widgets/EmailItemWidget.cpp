...
QLabel* makeRowLabel(QWidget* parent,
...
EmailItemWidget::EmailItemWidget(const EmailSummary& summary, QWidget* parent)
    : QWidget(parent)
    , m_summary(summary)
{
    createUi();
...
EmailItemWidget::EmailItemWidget(QWidget* parent)
    : QWidget(parent)
{
    createUi();
    updateContent();
...
void EmailItemWidget::createUi()
{
    ...
    mainLayout->setContentsMargins(MAIN_MARGIN, TOP_BOTTOM_MARGIN,
                                   MAIN_MARGIN, TOP_BOTTOM_MARGIN);
    ...
    m_senderLabel = makeRowLabel(this,
                                 QStringLiteral("SenderLabel"),
                                 TEXT_HEIGHT_LARGE,
                                 SENDER_FONT_SIZE);
    topRow->addWidget(m_senderLabel, 1);

    m_dateLabel = makeRowLabel(this,
    ...
    m_subjectLabel = makeRowLabel(this,
                                  QStringLiteral("SubjectLabel"),
                                  TEXT_HEIGHT_LARGE,
                                  SUBJECT_FONT_SIZE);
    contentLayout->addWidget(m_subjectLabel);

    m_previewLabel = makeRowLabel(this,
                                  QStringLiteral("PreviewLabel"),
                                  TEXT_HEIGHT_SMALL,
                                  PREVIEW_FONT_SIZE);
...
void EmailItemWidget::updateContent()
{
    ...
    {
        QString subject = m_summary.subject.isEmpty()
