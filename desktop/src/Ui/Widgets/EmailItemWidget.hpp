...
class EmailItemWidget : public QWidget
{
 ...
 public:
  explicit EmailItemWidget(const aurora::mail::app::email::EmailSummary& summary, QWidget* parent = nullptr);
  explicit EmailItemWidget(QWidget* parent = nullptr);
  ~EmailItemWidget() override = default;

  [[nodiscard]] const QString& uid() const noexcept
  {
    return m_summary.uid;
  }
  ...
  [[nodiscard]] bool isUnread() const noexcept
  {
    return !m_summary.isRead;
  }
  ...
  [[nodiscard]] bool showsRecipient() const noexcept
  {
    return m_showRecipient;
  }
